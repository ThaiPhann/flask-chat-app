...
def create_room():
    ...
    if request.method == 'POST':
        ...
        if remove_user:
            ...
            elif remove_user in members:
                members.remove(remove_user)
        ...
        if action == "search":
            ...
            if room_name and len(usernames) >= 2:
    ...
    if request.method == 'POST':
        ...
        if action == "search":
            ...
            if len(room_members) < 2:
                message = 'Room chat must have at least 2 members'
            else:
...
def handle_join_room_event(data):
    ...
    socketio.emit("online_users", list(online_users[room]), to=room)
...
def handle_leave_room_event(data):
    ...
    socketio.emit("leave_room_announcement", data, to=room)
    socketio.emit("online_users", list(online_users.get(room, [])), to=room)

@socketio.on("typing")
def handle_typing(data):
    socketio.emit("typing", data, to=data["room"], include_self=False)

@socketio.on("stop_typing")
def handle_stop_typing(data):
    socketio.emit("stop_typing", data, to=data["room"])
